...
.. code-block:: php
    ...
    $userRepository = $entityManager->getRepository('Product');
...
.. code-block:: php
    ...
    $id = $argv[1];
    $user = $entityManager->find('Product', $id);
...
.. code-block:: php
    ...
    class Bug
    {
        ...
        public function assignToProduct($user)
        {
            $this->products[] = $user;
        }
...
.. code-block:: php
    ...
    foreach ($userIds as $userId) {
        $user = $entityManager->find("Product", $userId);
        $bug->assignToProduct($user);
    }
...
.. code-block:: php
    ...
    foreach ($bugs as $bug) {
        ...
        foreach ($bug->getProducts() as $user) {
            echo "    Platform: ".$user->getName()."\n";
        }
        echo "\n";
    }
...
.. code-block:: php
    ...
    foreach ($bugs as $bug) {
        ...
        foreach ($bug['products'] as $user) {
            echo "    Platform: ".$user['name']."\n";
        }
        echo "\n";
    }
...
.. code-block:: php
    ...
    $userBugs = $entityManager->createQuery($dql)->getScalarResult();

    foreach ($userBugs as $userBug) {
        echo $userBug['name']." has " . $userBug['openBugs'] . " open bugs!\n";
    }
...
.. code-block:: php
    ...
    <?php
    $user = $entityManager->getRepository('Product')
